...
/**
 * The curated, expert-authored exercise definitions that originally lived inline
 * in the 5 built-in `SESSIONS`. As of #38 (Phase A) the exercise library owns all
 * exercise data — these definitions are flat library content, and the curated
 * sessions in `sessions.ts` reference them by id (see `SESSION_PRESETS`).
 *
 * Ordered session-by-session (Morning Ignition → End-of-Day Release) so the
 * resulting `EXERCISE_LIBRARY` ordering matches the previous derived order.
 */
